...
- **Attention triggers**: a card surfaces in Needs Attention on CI failing, a merged PR whose card isn't yet In Test/Done, or a new Jira comment while the card is In Test — except while its PR is a draft, which always routes to Self Review Needed instead. Other triggers (new comments on cards elsewhere, missing QA instructions or fix version) show as pills on the row without moving the card.
